...
def main(options):
    ...
    if program.use_Pushbullet:
        propyte.start_messaging_Pushbullet(token = program.Pushbullet_token)
...
def assess_Bitcoin():
    ...
    if abs(percentage_change_prediction_Bitcoin) > 1.5 or send_report_now:
        text = "linear prediction: Bitcoin value change by {percentage:.2f}% in ~2 days (from current value {value_current_Bitcoin:.2f} GBP to {value_prediction_Bitcoin:.2f} GBP)".format(
